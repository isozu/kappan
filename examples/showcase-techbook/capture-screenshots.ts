#!/usr/bin/env node
/**
 * showcase-techbook の横組みレイアウトを画像化するスクリーンショットスクリプト。
 *
 ...
 * screenshots/ に出力する。techbook.png は README で参照するヒーロー画像（第1章）。
 *
 * 用法:
...
const HERO_NAME = 'techbook.png';

async function main(): Promise<void> {
  const puppeteer = (await import('puppeteer-core')).default;
  const workDir = await mkdtemp(path.join(tmpdir(), 'kappan-showcase-techbook-'));

  try {
    // 1. ショーケース（showcase-techbook）をその場でビルド
    const { buildBook, loadConfig } = await import('@kappan/core');
    const configPath = path.join(HERE, 'kappan.config.ts');
    const { config, configDir } = await loadConfig(configPath);
    const epubPath = path.join(workDir, 'showcase-techbook.epub');
    await buildBook({
      config,
      configDir,
      outputPath: epubPath,
    ...
    console.error('[showcase-techbook] built EPUB for screenshots');
    ...
    try {
      for (const chapter of chapters) {
        ...
        await page.close();
        console.error(`[showcase-techbook] captured: ${base}.png`);
      }
    } finally {
      await browser.close();
      server.close();
    ...
    console.error(
      `\n[showcase-techbook] ✅ ${chapters.length} 章 + ${HERO_NAME} を ` +
