...
[@lst:build-entry]のとおり、著者が触れる API は驚くほど小さい。
...
shiki プラグインの骨格 {#lst:shiki-plugin}
...
[@lst:shiki-plugin]は `onHast` フックで `<pre>` 要素を捕まえ、
ハイライト済みのノードに差し替えている。
...
[@tbl:langs]の言語であれば、テーマ `github-light` で安定した配色になる。

ビルドと検証コマンド {#lst:build-cmd}
...
[@lst:build-cmd]のコマンド一発で、ビルドと標準検証が走る。
