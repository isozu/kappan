...
| 表番号     | 表の直前/直後にキャプション段落 `説明 {#tbl:id}`     | ✓           |
| リスト番号 | コードの直前/直後にキャプション段落 `説明 {#lst:id}` | ✓           |
| 節番号     | `## 節 {#sec:id}`                                    | ✓           |
| 数式番号   | `$$ ... $$` の直後段落に `{#eq:id}`                  | ✓           |
| 図表参照   | `[@fig:id]` / `[@tbl:id]` / `[@lst:id]`              | ✓（リンク） |
| 節・章参照 | `[@sec:id]` / `[@chap:id]`                           | ✓（リンク） |
| 数式参照   | `[@eq:id]`                                           | ✓（リンク） |

GFM ネイティブ table（`| ... |`）は、表の**直前または直後**に「キャプション本文 + `{#tbl:id}`」だけの段落を置くと採番される。コードブロックも同様に「キャプション本文 + `{#lst:id}`」の段落を隣接させると `リストN.N` のキャプション付き `<figure>` になる（後方互換として、コードブロック直後の段落**先頭**に `{#lst:id}` を置く旧記法も番号だけは振られる）。画像ベースの表（`![](表){#tbl:id}`）も従来どおり採番対象。
...
> 別章の図表・節を参照するときは `[@fig:ch03/overview]` のように `章ID/参照ID` で名前空間を切る。`章ID` は見出しの `{#chXX}` と front-matter `id` を一致させておく（章ファイル `chXX.xhtml` へのリンク先になる）。未定義の参照はビルド時に警告が出る。引用文献参照（`[@cite:id]`）は今後対応予定。
